...
# Prefer the new langchain-ollama provider, fallback to community for compatibility
...
async def llm_json_typed(system: str, user: str, schema: Type[BaseModel], *, model: str | None = None, timeout: int | None = None) -> BaseModel:
    """Like llm_json, but validates against a Pydantic schema and retries on parse/validation errors."""
    cfg = get_llm_config()
    last_err: Exception | None = None
    for attempt in range(cfg.max_retries + 1):
        try:
